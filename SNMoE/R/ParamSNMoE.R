  ...
  methods = list(
    initParam = function(modelSNMoE, phiAlpha, phiBeta, try_EM, segmental = FALSE) {
      ...
      if (segmental == FALSE) {
        ...
        klas <- floor(modelSNMoE$K * matrix(runif(modelSNMoE$n*modelSNMoE$m),modelSNMoE$n))+1
      ...
      if (try_EM == 1) {
        param$alpha <- zeros(modelSNMoE$q+1, modelSNMoE$K-1)
      }
    ...
    MStep = function(modelSNMoE, statSNMoE, phiAlpha, phiBeta, verbose_IRLS) {
      # M-Step
      res_irls <- IRLS(phiBeta$XBeta, statSNMoE$tik, alpha, verbose_IRLS = verbose_IRLS)
      piik <- res_irls$piik
      alpha <<- res_irls$W

      for (k in 1:modelSNMoE$K) {
        #update the regression coefficients
        tauik_Xbeta <- (statSNMoE$tik[,k] %*% ones(1, modelSNMoE$p+1))*phiAlpha$XBeta
        beta[,k] <<- solve((t(tauik_Xbeta)%*%phiAlpha$XBeta)) %*% ( t(tauik_Xbeta) %*% (modelSNMoE$Y - delta[k] * statSNMoE$E1ik[,k]))

        # update the variances sigma2k
        sigma[k] <<- sum(statSNMoE$tik[,k] * ((modelSNMoE$Y - phiAlpha$XBeta %*% beta[,k])^2 - 2 * delta[k] * statSNMoE$E1ik[,k] * (modelSNMoE$Y - phiAlpha$XBeta %*% beta[,k]) + statSNMoE$E2ik[,k])) / (2 * (1 - delta[k]^2) * sum(statSNMoE$tik[,k]))


        # update the deltak (the skewness parameter)
        delta[k] <<- uniroot(f <- function(dlt) {
          sigma[k] * dlt * (1-dlt^2)*sum(statSNMoE$tik[,k]) + (1+ dlt^2) * sum(statSNMoE$tik[,k] * (modelSNMoE$Y-phiAlpha$XBeta%*%beta[,k])*statSNMoE$E1ik[,k])
          - dlt * sum(statSNMoE$tik[,k]*(statSNMoE$E2ik[,k] + (modelSNMoE$Y - phiAlpha$XBeta %*% beta[,k])^2))
        }, c(-1, 1))$root


        #uniroot(f <- function(dlt) {
        #  param$sigma[k] * dlt * (1-dlt^2)*sum(stat$tik[,k]) + (1+ dlt^2) * sum(stat$tik[,k] * (modelSNMoE$Y-phiAlpha$XBeta%*%param$beta[,k])*stat$E1ik[,k])
        #       - dlt * sum(stat$tik[,k]*(stat$E2ik[,k] + (modelSNMoE$Y - phiAlpha$XBeta %*% param$beta[,k])^2))
        #   }, c(-1, 1))
        ...
        #  - dlt * sum(stat$tik[,k]*(stat$E2ik[,k] + (modelSNMoE$Y - phiAlpha$XBeta %*% param$beta[,k])^2))
        #}
        #plot(vecf, sapply(vecf, f), type = "l")
        ###########################################################################################################################
        ###########################################################################################################################
